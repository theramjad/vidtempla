...
export const syncChannelVideos = task({
  ...
  onFailure: async ({ payload, error }) => {
    ...
    logger.error("Sync failed for channel", { channelId, error: errorMessage });
  ...
  run: async (payload: { channelId: string; userId: string }) => {
    ...
    // Update channel info (subscriber count, etc.)
    try {
    ...
    // Fetch all videos from YouTube (handle pagination)
    ...
    // Update last_synced_at timestamp and set status to idle
    await db
      .update(youtubeChannels)
      .set({
